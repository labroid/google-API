...
def main():
    ...
    while True:
        if file_list is None:
            ...
            if file_list['nextPageToken'] is None:
                break
        ...
        for f in file_list['items']:
            traversed += 1
            file_id = '1el16TSNYvaGQndXXDZjheN_CANnWbA9wSA'
#            md = service.files().get(fileId=f['id'], fields="id, md5Checksum, properties").execute()
#            md = service.files().get(fileId=f['id']).execute()
            md = service.files().get(fileId = file_id).execute()
            print "files().get() returns:"
            print json.dumps(md, indent=4)
            md = service.properties().list(fileId=file_id).execute()
            ...
            #         {
            #             "key": "md5sum",
            #             "value:": md['md5Checksum']
            #         }
            #     ]
            # }
     #       foo = service.files().patch(fileId=md['id'],body=update).execute()
     #       print type(foo)
    ...
    # tree_root = service.children().list(folderId='root', q="title = 'Google Photos'", fields="items/id").execute()
    # print "Tree Root:", tree_root
    # top_level = service.children().list(folderId=tree_root['items'][0]['id'], fields="items/id").execute()
    # print "Top Level:", json.dumps(top_level['items'])
    # for d in top_level['items']:
    #     dstats = service.files().get(fileId=d['id'], fields="mimeType, id, title").execute()
    #     print dstats
    ...
    # results = service.files().list(q="title = 'IMG_1773.JPG'", maxResults=10).execute()
    # print type(results)
    # print results
